...
class Batch_Balanced_Dataset(object):
    ...
    def __init__(self, opt):
        """
         バッチ内のデータ比率を変化させます。
         たとえば、select_dataが「MJ-ST」で、batch_ratioが「0.5-0.5」の場合、
         バッチの50％はMJで満たされ、バッチの残りの50％はSTで満たされます。
        """
        print('-' * 80)
        ...
        for selected_d, batch_ratio_d in zip(opt.select_data, opt.batch_ratio):
            ...
            """
             データの総数はopt.total_data_usage_ratioで変更できます。
             例）opt.total_data_usage_ratio = 1は100％の使用率を示し、0.2は20％の使用率を示します。
             論文の4.2セクションを参照してください。
            """
...
def hierarchical_dataset(root, opt, select_data='/'):
    """ select_data = '/'には、ルートディレクトリのすべてのサブディレクトリが含まれます """
    dataset_list = []
...
class LmdbDataset(Dataset):
        ...
        with self.env.begin(write=False) as txn:
            ...
            if self.opt.data_filtering_off:
                ...
                for index in range(self.nSamples):
                    ...
                    # デフォルトでは、opt.characterにない文字を含む画像はフィルターされます。
                    # このフィルタリングの代わりに、utils.pyの「opt.character」に[UNK]トークンを追加できます。
    ...
    def __getitem__(self, index):
        ...
        with self.env.begin(write=False) as txn:
            ...
            try:
                if self.opt.rgb:
                    img = Image.open(buf).convert('RGB')  # カラー画像用
                else:
            ...
            # 英数字（またはtrain.pyで事前定義された文字セット）についてのみトレーニングおよび評価します
            out_of_char = f'[^{self.opt.character}]'
